...
export default function AdminScreen({ st, setSt, showToast }) {
  ...
  const approve = (entry) => {
    ...
    setSt(s => ({
      ...
      _cancelledIds: [...new Set([...(s._cancelledIds || []), entry.id])],
      sergei: { ...s.sergei, coins: s.sergei.coins + task.reward, totalEarned: (s.sergei.totalEarned || 0) + task.reward, completedTasks: [...s.sergei.completedTasks, { taskId: task.id, date: Date.now() }], log: [{ id: crypto.randomUUID(), type: "earn", text: `✅ Задание «${task.title}» одобрено`, amount: task.reward, ts: Date.now() }, ...s.sergei.log].slice(0, 100) },
    }));
  ...
  const reject = (entry) => {
    ...
    setSt(s => ({
      ...
      pendingTasks: s.pendingTasks.filter(p => p.id !== entry.id),
      _cancelledIds: [...new Set([...(s._cancelledIds || []), entry.id])],
      sergei: { ...s.sergei, log: [{ id: crypto.randomUUID(), type: "reject", text: `❌ Задание «${task?.title || "—"}» отклонено`, ts: Date.now() }, ...s.sergei.log].slice(0, 100) },
    }));
      ...
      {tab === "rewards" && (
        <div style={{ background: "linear-gradient(135deg,#0f172a,#020617)", border: "1px solid #1e3a5f", borderRadius: 20, padding: 16 }}>
          ...
          <div style={{ background: "#07111f", border: "1px solid #1e3a5f", borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setNewReward(p => ({ ...p, oneTime: false }))} style={{ flex: 1, padding: "10px 0", borderRadius: 10, background: !newReward.oneTime ? "#059669" : "#0f172a", color: !newReward.oneTime ? "#fff" : "#475569", fontWeight: 800, fontSize: 13, cursor: "pointer", border: !newReward.oneTime ? "none" : "1px solid #1e3a5f" }}>
                ♾️ Неограниченно
              </button>
              <button onClick={() => setNewReward(p => ({ ...p, oneTime: true }))} style={{ flex: 1, padding: "10px 0", borderRadius: 10, background: newReward.oneTime ? "#f59e0b" : "#0f172a", color: newReward.oneTime ? "#020617" : "#475569", fontWeight: 800, fontSize: 13, cursor: "pointer", border: newReward.oneTime ? "none" : "1px solid #1e3a5f" }}>
                1️⃣ Одноразово
              </button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 20 }}>
            ...
            {st.rewards.map(r => (
              <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #0f172a" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => setSt(s => ({ ...s, rewards: s.rewards.filter(x => x.id !== r.id) }))} style={{ padding: "4px 10px", background: "#2d0a0a", color: "#f87171", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {tab === "tasks" && (
        <div style={{ background: "linear-gradient(135deg,#0f172a,#020617)", border: "1px solid #1e3a5f", borderRadius: 20, padding: 16 }}>
          ...
          <div style={{ marginTop: 20 }}>
            ...
            {st.tasks.map(t => (
              <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #0f172a" }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", minWidth: 0, flex: 1 }}>
                  <span style={{ fontSize: 18, flexShrink: 0 }}>{t.emoji}</span>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: "#f1f5f9" }}>{t.title}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  ...
                  <button onClick={() => setSt(s => ({ ...s, tasks: s.tasks.filter(x => x.id !== t.id) }))} style={{ padding: "4px 10px", background: "#2d0a0a", color: "#f87171", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {tab === "tiers" && (
        <div>
          <div style={{ background: "linear-gradient(135deg,#0f172a,#020617)", border: "1px solid #4c1d9555", borderRadius: 20, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "#475569", fontWeight: 700, marginBottom: 8, marginTop: -4 }}>
              💡 Загрузи .glb файл на любой хостинг (GitHub Raw, CDN) и вставь ссылку
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <button onClick={addCustomTier} style={{ width: "100%", padding: 14, background: "#a855f7", color: "#fff", border: "none", borderRadius: 14, fontWeight: 800, cursor: "pointer", marginTop: 4 }}>
              ✨ Добавить тир
            </button>
          </div>
              ...
              <div key={tier.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #0f172a" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#475569" }}>
                    {tier.modelUrl ? "🎲 3D-модель" : `${tier.emoji} Эмодзи`} • 💰 {tier.cost}
                  </div>
                </div>
                <button onClick={() => setSt(s => ({ ...s, customTiers: (s.customTiers || []).filter(x => x.id !== tier.id) }))} style={{ padding: "4px 10px", background: "#2d0a0a", color: "#f87171", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>✕</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background: "linear-gradient(135deg,#1c0a00,#2d1500)", border: "1px solid #78350f", borderRadius: 20, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 12, color: "#f59e0b", fontWeight: 700 }}>Доступны в магазине:</span>
            ...
            {currencyShop.chocolate.enabled && (
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input type="number" defaultValue={currencyShop.chocolate.price} onBlur={e => { const v = parseInt(e.target.value); if (v > 0) setSt(s => ({ ...s, currencyShop: { ...s.currencyShop, chocolate: { ...s.currencyShop.chocolate, price: v } } })); }} style={{ flex: 1, padding: "10px 14px", background: "#07111f", border: "1px solid #78350f", borderRadius: 12, color: "#f1f5f9", fontFamily: "'Nunito',sans-serif", fontSize: 14, outline: "none" }} />
                  <span style={{ display: "flex", alignItems: "center", color: "#fbbf24", fontWeight: 900, fontSize: 16 }}>💰</span>
                </div>
              </div>
            )}
            <div style={{ color: "#92400e", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>Текущий баланс: {st.sergei.chocolates || 0} 🍫</div>
            <div style={{ display: "flex", gap: 8 }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#0a0a2e,#1a1060)", border: "1px solid #4c1d95", borderRadius: 20, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 12, color: "#c084fc", fontWeight: 700 }}>Доступны в магазине:</span>
            ...
            {currencyShop.star.enabled && (
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input type="number" defaultValue={currencyShop.star.price} onBlur={e => { const v = parseInt(e.target.value); if (v > 0) setSt(s => ({ ...s, currencyShop: { ...s.currencyShop, star: { ...s.currencyShop.star, price: v } } })); }} style={{ flex: 1, padding: "10px 14px", background: "#07111f", border: "1px solid #4c1d95", borderRadius: 12, color: "#f1f5f9", fontFamily: "'Nunito',sans-serif", fontSize: 14, outline: "none" }} />
                  <span style={{ display: "flex", alignItems: "center", color: "#fbbf24", fontWeight: 900, fontSize: 16 }}>💰</span>
                </div>
              </div>
            )}
            <div style={{ color: "#7c3aed", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>Текущий баланс: {st.sergei.stars || 0} ⭐️</div>
            <div style={{ display: "flex", gap: 8 }}>
