...
export default function TasksScreen({ st, setSt, showToast }) {
  ...
  const submitTask = (task) => {
    // Уже ожидает проверки
    if (pendingIds.includes(task.id)) {
      return showToast("Уже отправлено на проверку", "info");
    }
    // Выполнено сегодня: для одноразовых — запрещено, для повторных —
    // нужно сначала получить одобрение предыдущего
    if (completedToday.includes(task.id)) {
      return showToast(
        task.repeatable ? "Подожди одобрения перед повторной отправкой" : "Уже выполнено сегодня",
        "info"
      );
    }
    const entry = {
    ...
    setSt(s => ({
      ...
      sergei: {
        ...s.sergei,
        log: [
          { id: crypto.randomUUID(), type: "submit", text: `📤 Отправил задание «${task.title}»`, ts: Date.now() },
          ...s.sergei.log,
        ].slice(0, 100),
      },
    }));
  ...
  const cancelTask = (task) => {
    setSt(s => {
      const removed = s.pendingTasks.filter(p => p.taskId === task.id && p.userId === "sergei");
      const removedIds = removed.map(p => p.id);
      return {
        ...s,
        pendingTasks: s.pendingTasks.filter(p => !(p.taskId === task.id && p.userId === "sergei")),
        _cancelledIds: [...new Set([...(s._cancelledIds || []), ...removedIds])],
        sergei: {
          ...s.sergei,
          log: [
            { id: crypto.randomUUID(), type: "cancel", text: `↩️ Отменил задание «${task.title}»`, ts: Date.now() },
            ...s.sergei.log,
          ].slice(0, 100),
        },
      };
      ...
      <div
        ...
        style={{
          background: isDoneToday
            ? "linear-gradient(135deg,#031a10,#042a18)"
            : isPending
            ? "linear-gradient(135deg,#1c1407,#120c00)"
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {task.description && (
              <div style={{ fontSize: 12, color: "#475569", fontWeight: 600, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {task.description}
              </div>
            )}
            <div style={{ fontSize: 11, color: "#334155", fontWeight: 700 }}>
              {task.category} •{" "}
              {task.difficulty === "easy" ? "🟢 Лёгкое" : task.difficulty === "medium" ? "🟡 Среднее" : "🔴 Сложное"}
              {task.repeatable ? " • 🔁" : ""}
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
            ...
            {isDoneToday ? (
              <span style={{ color: "#4ade80", fontWeight: 800, fontSize: 11 }}>✅ Готово</span>
            ) : isPending ? (
              <span style={{ color: "#fbbf24", fontWeight: 800, fontSize: 11 }}>⏳ Проверка</span>
            ) : (
  ...
  return (
    <div style={{ padding: "20px 16px", paddingBottom: 100 }}>
      ...
      <div style={{ fontFamily: "'Baloo 2',sans-serif", fontSize: 22, fontWeight: 900, color: "#f1f5f9", marginBottom: 16 }}>
        📋 Задания
      </div>
        ...
        {categories.map(c => (
          <button
            ...
            style={{
              ...
              padding: "6px 16px",
              borderRadius: 99,
              border: filter === c ? "none" : "1px solid #1e3a5f",
              background: filter === c ? "#0ea5e9" : "#0f172a",
              color: filter === c ? "#020617" : "#475569",
              fontFamily: "'Nunito',sans-serif",
              fontWeight: 800,
              fontSize: 12,
              cursor: "pointer",
              boxShadow: filter === c ? "0 2px 14px #38bdf855" : "none",
            }}
          >
            {c}
          </button>
      ...
      {activeTasks.length === 0 && completedTasks.length === 0 && (
        <div style={{ textAlign: "center", padding: 40, color: "#334155" }}>
          ...
          <div style={{ fontWeight: 700 }}>Нет заданий</div>
        </div>
      )}
      ...
      {completedTasks.length > 0 && (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: "#166534", textTransform: "uppercase", letterSpacing: ".05em" }}>
